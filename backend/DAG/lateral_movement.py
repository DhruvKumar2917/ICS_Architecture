...
Lateral movement analysis detects HOW an attacker moves sideways
between systems after initial compromise — specifically:

  1. Cross-zone movement  — attacker crosses ISA-62443 security zones
  2. High privilege transitions — gaining elevated access mid-path
  3. Remote access chains — VPN/SSH chains linking distant systems
  4. Purdue level violations — skipping Purdue model layers
  5. Protocol pivots — switching protocols to evade detection

Real-world ICS attacks exhibiting lateral movement:
  - Stuxnet:    moved from IT network → engineering workstation → SCADA → PLC
  - Triton:     moved from corporate → DMZ → engineering → safety controller
  - Industroyer: moved from C2 → RTU via industrial protocols

Usage:
    from DAG.lateral_movement import LateralMovementAnalyzer
    analyzer = LateralMovementAnalyzer(ics_graph)
...
def _parse_purdue_level(level_str: str) -> Optional[int]:
    """Extract integer Purdue level from 'Level N' string."""
    if not level_str:
        return None
    try:
...
class LateralMovementAnalyzer:
    """
    Detects lateral movement patterns in ICS Security Graphs.

    Analyses all pairs of connected nodes and classifies each
    connection by movement type. Also reconstructs full lateral
    movement chains from entry points to critical targets.
    """
    ...
    # ------------------------------------------------------------------ #
    # Detection rules (applied per edge)
    # ------------------------------------------------------------------ #

    def _detect_cross_zone(self, u: str, v: str, edge_data: Dict) -> Optional[Dict]:
        """Rule 1: Detect movement between different security zones, identifying IT-to-OT pivots."""
        ...
        if u_zone and v_zone and u_zone != v_zone:
            ...
            # Standard cross-zone movement
            severity = "HIGH"
            # Raise severity to CRITICAL if moving into safety zone or target is safety controller
    ...
    def _detect_privilege_escalation(self, u: str, v: str, edge_data: Dict) -> Optional[Dict]:
        """Rule 2: Detect movement from low-privilege to high-privilege node."""
    ...
    def _detect_remote_access_chain(self, u: str, v: str, edge_data: Dict) -> Optional[Dict]:
        """Rule 3: Detect remote access or VPN traversal edges."""
    ...
    def _detect_purdue_violation(self, u: str, v: str, edge_data: Dict) -> Optional[Dict]:
        """Rule 4: Detect Purdue level violations (layer skipping)."""
        u_purdue = _parse_purdue_level(self.asset_graph.nodes[u].get("purdue_level"))
        v_purdue = _parse_purdue_level(self.asset_graph.nodes[v].get("purdue_level"))
    ...
    def _detect_protocol_pivot(
        ...
        """
        Rule 5: Detect protocol pivot — switching from IT to OT protocol mid-path.
        Requires access to a full edge sequence (path-level analysis).
        """
        pivots = []
        if len(path_edges) < 2:
            return pivots
    ...
    def analyze_path_lateral_movement(self, path: List[str]) -> Dict[str, Any]:
        """
        Analyse a single attack path for all lateral movement patterns.

        Returns:
            {
                "path": [...],
                "movements": [...],   # detected movement events
                "movement_types": [...],  # unique types found
        ...
        # Apply path-level protocol pivot detection
    ...
    def analyze(self, top_paths: int = 5) -> Dict[str, Any]:
        """
        Full lateral movement analysis on the ICS graph.

        Enumerates paths from entry points to critical assets and
        analyses each for lateral movement patterns.

        Returns:
            {
                "movement_events": [...],         # all detected events
                "cross_zone_count": int,
                "privilege_escalation_count": int,
                "remote_chain_count": int,
                "purdue_violation_count": int,
                "protocol_pivot_count": int,
                "high_risk_paths": [...],         # paths with most movement events
                "summary": {...},
            }
        """
        ...
        # Edge-level scan (covers ALL edges, not just attack paths)
        ...
        # Path-level scan (protocol pivot requires full path context)
        if entries and targets:
            for entry in entries[:5]:   # cap to 5 to avoid explosion
                    ...
                    try:
                        ...
                        for path in islice(gen, top_paths):
                            ...
                            if report["is_lateral"]:
                                ...
                                # Add protocol pivots to global events (de-duplicated)
        ...
        # Deduplicate events by (type, from, to)
        ...
        # Sort path reports by movement count
