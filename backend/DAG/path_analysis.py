...
class ICSPathAnalyzer:
    ...
    def get_edge_propagation_prob(self, u: str, v: str) -> float:
        """
        Estimate the probability of threat propagation from u to v.
        Uses edge types, protocols, zone crossings, Purdue levels, and firewall defenses.
        """
        ...
        # 1. Base weights per edge type / privilege action
        base_weights = {
            "COMM_LINK":      0.9,
            "CYBER_PHYSICAL": 0.5,
            "HUMAN_PERM":     0.4
        ...
        if edge_type == "HUMAN_PERM":
            ...
            if any(p in label for p in _privileged):
                prob = 0.8  # Highly privileged control permissions carry high compromise likelihood if credentials are obtained
                
        # 2. Protocol security modifiers
        ...
        # 6. Purdue Level movement penalty (moving downward/deeper into OT levels)
        u_p = self._parse_purdue_level(u_attrs.get("purdue_level"))
        v_p = self._parse_purdue_level(v_attrs.get("purdue_level"))
    ...
    def reconstruct_attack_path_details(self, path):
        """
        Translates a list of node IDs into a detailed attack path trace with step descriptions.
        """
        steps = []
        i = 0
        while i < len(path) - 1:
            ...
            # Check if there is a direct edge in the asset graph
            if self.asset_graph.has_edge(u, v):
                ...
                # Check for credential compromise / privilege escalation (u -> s -> w)
                if i + 2 < len(path):
                    s = path[i+1]
                    w = path[i+2]
                ...
                # Fallback path step description
    ...
    def estimate_physical_disruption(self, node: str) -> dict:
        """
        Estimate the operational disruption on a physical asset if it is compromised.
        """
        attrs = self.asset_graph.nodes[node]
    ...
    def analyze_blast_radius(
        ...
        """
        Research-grade, attack-path-aware, weighted cyber-physical blast radius.
        Computes the Weighted Blast Radius Score (WBRS) reflecting operational consequences.
        """
        ...
        # Traversal respecting hops and enforcement points, tracking probabilities and paths
        visited = {}
        queue = []
        ...
        # BFS traversal with privilege escalation
        while queue:
            u, hop_dist, u_prob, u_path = queue.pop(0)
            if hop_dist >= max_hops:
                continue

            # 1. Direct successors (Network Comm / Cyber-Physical links)
            for v in self.asset_graph.successors(u):
                ...
                if next_prob < 0.05:  # Prune low probability noise
                    continue
            ...
            # 2. Privilege Escalation (Credential stealing & reuse)
            # Find operators/subjects logging into asset u
            for s in self.asset_graph.predecessors(u):
                if self.asset_graph[s][u].get("edge_type") == "HUMAN_PERM":
                    # Attacker compromises credentials of subject s
                    # Pivot to any other asset w that s has permission to access
        ...
        # Filter out origin nodes from reachable results
        ...
        for node, info in reachable.items():
            ...
            # Incorporate Critical Asset Weighting & Purdue level scaling
            v_type = str(attrs.get("type", "")).lower()
            category = attrs.get("node_category", "")

            # 1. Base weights by Asset Type
            ...
            # Scale impact score by hops decay and threat propagation success probability
            impact_score = round(w_v * (decay ** info["hop"]) * info["prob"], 2)

            # Attack Path Tracer Details
            ...
            # Physical Disruption estimation
        ...
        # Run Blast Radius Validation Agent
...
class BlastRadiusValidationAgent:
    """
    Automated validation agent that cross-validates consistency between blast radius,
    filtered graph reachability, and attack paths.
    """
    ...
    def validate(self, origin_node, exposed_nodes_ranked, max_hops=6):
        ...
        1. Reachability: Every node in blast radius must be reachable from origin node.
        2. Attack Paths: Checks if every node in blast radius appears in at least one valid attack path.
        3. Critical Asset Coverage: Checks that any critical asset reachable by attack paths from origin is also in the blast radius.
        """
        errors = []
        warnings = []
        is_consistent = True
        ...
        # 1. Structural reachability check
        reachability_status = {}
        ...
        # Flatten all nodes traversed in any of these attack paths
        ...
        # 3. Critical Target coverage check
    ...
    def simulate_remediation_impact(
        ...
        """
        Simulate the removal of firewalls, VPNs, or communication links
        and measure the reduction in the total blast radius score.
        """
