...
# ---------------------------------------------------------------------------
# Multi-Agent Reasoning Pipeline Components
# ---------------------------------------------------------------------------

class ContextBuilderAgent:
    """
    Agent responsible for enriching every graph edge with complete attack context.
    """
    def __init__(self):
        pass
    ...
    def enrich_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        Takes raw context from extract_context and returns an enriched dict
        with keys defined for the reasoning agent.
        """
...
class CandidateSelectionAgent:
    """
    Deterministic rule engine that filters the MITRE ATT&CK for ICS catalog
    and produces only a small set of semantically valid candidate techniques.
    """
    def __init__(self):
        pass
    ...
    def select_candidates(self, context: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        # 1. Base candidates list from official KB (technique ID and metadata)
        all_kb_techniques = []
        ...
        # 2. Filter by asset compatibility
        asset_filtered = []
        ...
        # 3. Filter by action compatibility using the matrix
        action_filtered = []
        if action_norm in ACTION_TECHNIQUE_COMPATIBILITY_MATRIX:
            ...
            # Heuristic match if not exact match in matrix
        ...
        # 4. Filter by protocol if applicable
        protocol_filtered = []
        ...
        # Convert back to simple dict output
        result = []
...
Your selected technique, its tactic, and the reason MUST describe the same adversarial behavior. The justification (reason) must explain the final selected technique, not the original action alone. (e.g. if the technique is Valid Accounts, explain why Valid Accounts was chosen and do NOT discuss Remote Services. If the technique is Remote Services, explain why Remote Services was chosen and do NOT discuss Valid Accounts).
...
class MITREReasoningAgent:
    """
    Agent responsible for selecting and ranking candidates from the shortlist
    and providing an explainable justification.
    """
...
class SemanticValidationAgent:
    """
    Agent responsible for verifying Action <-> Technique <-> Asset <-> Protocol <-> Explanation
    consistency as a single unit, rejecting any mapping where these elements contradict one another.
    """
    def __init__(self):
        pass
    ...
    def validate(
        ...
        # 2. Check Candidate short-list compatibility
        ...
        # 3. Deterministic Action-Technique compatibility check
        action = str(context.get("action", "")).lower()
        action_norm = action.replace("-", "_").replace(" ", "_")
        if action_norm in ACTION_TECHNIQUE_COMPATIBILITY_MATRIX:
            allowed_ids = ACTION_TECHNIQUE_COMPATIBILITY_MATRIX[action_norm]
        ...
        # 4. Target Asset compatibility check
        ...
        # 6. Explanation-Technique contradiction checks (coupling reasoning to technique)
        ...
        # 7. Firewall and Reachability outcome contradiction checks
        ...
        # 8. Check Tactic matching candidates
...
# ---------------------------------------------------------------------------
# Additional Multi-Agent Reasoning Pipeline Components
# ---------------------------------------------------------------------------

class AttackChainReasoningAgent:
    """
    Classifies the edge's role/stage within the overall attack sequence.
    """
    def __init__(self):
        pass
...
class KnowledgeBaseVerificationAgent:
    """
    Cross-checks every selected technique against the official ATT&CK for ICS KB.
    """
    def __init__(self):
...
class ConfidenceCalibrationAgent:
    """
    Computes a composite confidence score using multiple evidence inputs.
    """
    ...
    def calibrate(
        ...
        # 1. Reachability (weight 15%): Yes (+0.15), No (-0.20)
        ...
        # 2. Firewall Status (weight 15%): Allowed (+0.15), Denied (-0.20)
        ...
        # 4. Asset Compatibility (weight 10%)
        target_type = str(context.get("object_type", "")).lower()
        if tech_id in self.kb:
            kb_entry = self.kb[tech_id]
        ...
        # Rigid clamps for firewall/reachability failure
        if firewall == "Denied":
...
class GraphConsistencyAgent:
    """
    Ensures that the tactic rank does not violate progression flow constraints.
    """
    def __init__(self):
        pass
...
def get_candidate_techniques(context: Dict[str, Any]) -> List[Dict[str, str]]:
    """
    Deterministic rule engine that returns a shortlist of valid MITRE ATT&CK for ICS
    techniques based on target asset type, protocol, and action category.
    """
...
def extract_context(
    ...
    previous_node : str
        ID of the preceding node in the attack chain.
    next_node : str
        ID of the succeeding node in the attack chain.
    ...
    dict with keys:
        ...
        source_zone, target_zone, criticality, previous_node, next_node
    """
...
def validate_mapping(
    ...
    # Enforce technique fallback if technique is completely invalid/not in candidates
    tech_id = str(mapping.get("technique_id", "")).upper().strip()
    candidate_ids = {c["id"] for c in candidates}
    
    if tech_id not in candidate_ids:
        # Override with fallback from candidate list to guarantee compliance
        if candidates:
    ...
    # Normalize/ensure KB attributes match officially
    if tech_id in ATTACK_ICS_KB:
        kb_entry = ATTACK_ICS_KB[tech_id]
    ...
    # Programmatic separation of execution outcome
    ...
    # Evidence trace mapping if not present
...
class LLMMITREMapper:
    ...
    def map_single_edge(
        ...
        """
        Map a single AASG edge using the multi-stage reasoning pipeline with feedback retries.
        """
        # 1. Context Builder Agent
        ...
        # 3. Attack Chain Reasoning Agent
        ...
        # 4. MITRE Reasoning Agent (with retry loop)
        ...
        for attempt in range(max_attempts):
            ...
            # Run sequential validation layers
            ...
            if not all_errors:
                ...
                # Format feedback to guide the model on self-correction
        ...
        # Call validate_mapping to get final attributes, confidence, warnings and traces
    ...
    def map_attack_path(
        ...
        Uses the multi-stage pipeline for each edge in the path sequentially,
        maintaining tactic state to ensure graph consistency, then runs formal_analysis().
        """
